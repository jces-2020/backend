...
def add_cliente():
    ...
    existe_correo = supabase.table('cliente').select('id_cliente').eq('correo', correo).limit(1).execute()
    if existe_correo.data:
        return jsonify({'success': False, 'message': 'El correo ya está registrado.'}), 409

    if documento:
        existe_documento = supabase.table('cliente').select('id_cliente').eq('documento', documento).limit(1).execute()
        if existe_documento.data:
            return jsonify({'success': False, 'message': 'Ya existe un cliente registrado con este DNI/documento.'}), 409

    if nombre:
        existe_nombre = supabase.table('cliente').select('id_cliente').ilike('nombre', nombre).limit(1).execute()
