...
class ClienteService(BaseService):
    ...
    def crear_cliente(self, data: Dict[str, Any]) -> Cliente:
        ...
        # 4. Verificar que nombre no exista
        nombre = data.get('nombre', '').strip()
        if nombre and self.repository.find_by_nombre_exacto(nombre):
            raise DuplicateEntityException("Cliente", "nombre", nombre)
    ...
    def actualizar_cliente(self, cliente_id: str, data: Dict[str, Any]) -> Cliente:
        ...
        if nuevo_nombre:
            ...
            if nuevo_nombre != cliente.nombre:
                otro = self.repository.find_by_nombre_exacto(nuevo_nombre)
                if otro:
                    raise DuplicateEntityException("Cliente", "nombre", nuevo_nombre)
