...
def send_payment_notification_batch(
    recipients,
    monto: float,
    tipo: str = "mensual",
    ...
    """
    Envía una notificación de pago a una lista de destinatarios.
    Recipients debe ser una lista de dicts con keys 'to' y 'nombre'.
    """
    stats = {
        "sent": 0,
        "failed": 0,
    ...
    for recipient in recipients:
        try:
            correo = recipient.get("correo") or recipient.get("to")
            nombre = recipient.get("nombre") or ""
            if not correo:
                raise ValueError("Falta correo del destinatario")

            if group_message:
                if tipo == "bono":
                    asunto = "Notificación de pago de bono para todo el personal"
                    descripcion = (
                        f"Se ha registrado un pago de bono por un monto total de <strong>S/ {monto:.2f}</strong> "
                        "para todo el personal."
                    )
                else:
                    asunto = "Notificación de pago mensual para todo el personal"
                    descripcion = (
                        f"Se ha registrado un pago mensual por un monto total de <strong>S/ {monto:.2f}</strong> "
                        "para todo el personal."
                    )
                      ...
                      <td align="center">
                        <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
                          <tr>
                            <td style="background:#4f46e5;padding:32px 40px;text-align:center;">
                              <div style="font-size:36px;margin-bottom:8px;">💼</div>
                              <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700;letter-spacing:-0.3px;">{asunto}</h1>
                            </td>
                          </tr>
                          <tr>
                            <td style="padding:36px 40px;">
                                ...
                                <tr>
                                  <td style="padding:20px 24px;">
                                    <table width="100%" cellpadding="0" cellspacing="0">
                                      ...
                                      <tr>
                                        <td style="border-top:1px solid #e5e7eb;padding-top:12px;color:#374151;font-size:15px;font-weight:700;">MONTO TOTAL</td>
                                        <td style="border-top:1px solid #e5e7eb;padding-top:12px;color:#4f46e5;font-size:18px;font-weight:800;text-align:right;">S/ {monto:.2f}</td>
                                      </tr>
                                    </table>
                                  </td>
                ...
                text_body = (
                    f"Hola {nombre},\n\n"
                    f"Se ha registrado un pago {'de bono' if tipo == 'bono' else 'mensual'} por un monto total de S/ {monto:.2f} para todo el personal.\n\n"
                    f"Este es un mensaje automático."
                )

                send_email(to=correo, subject=asunto, text_body=text_body, html_body=html_body)
            else:
                send_payment_notification(to=correo, nombre=nombre, monto=monto, tipo=tipo)
