...
def _crear_almacen(fila: Optional[str], columna: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    Returns:
        Tupla (id_almacen_creado, mensaje_error). Antes esta función se
        tragaba cualquier error (solo hacía print y devolvía None), así que
        si Supabase rechazaba el insert (p. ej. por una política RLS en la
        tabla 'almacen'), el producto se guardaba igual pero sin ubicación,
        sin que nadie se enterara del motivo. Ahora se propaga el error para
        poder mostrarlo como advertencia en la respuesta.
    """
    ...
    try:
        ...
        if err_alm:
            ...
            return None, str(err_alm)

        if isinstance(data_alm, list) and len(data_alm) > 0:
            return data_alm[0].get('id_almacen'), None
        elif isinstance(data_alm, dict):
            return data_alm.get('id_almacen'), None

        return None, "La inserción en 'almacen' no devolvió datos"
    except Exception as e:
...
def _adjuntar_ubicacion_almacen(producto: Dict[str, Any]) -> None:
    ...
    que la ubicación quede visible (antes solo se guardaba, nunca se leía).

    Modifica `producto` in-place; no falla el request si la lectura falla.
    """
    almacen_id = producto.get('almacen_id') if producto else None
    if not almacen_id:
        return
    try:
        resp = supabase.table('almacen').select('fila, columna').eq('id_almacen', almacen_id).limit(1).execute()
        data = getattr(resp, 'data', None) or []
        if data:
            producto['fila'] = data[0].get('fila')
            producto['columna'] = data[0].get('columna')
    except Exception as e:
        print(f"Excepción leyendo ubicación de almacén: {str(e)}")
...
def registrar_producto():
    ...
    try:
        ...
        if data.get('fila') or data.get('columna'):
            almacen_id, almacen_error = _crear_almacen(data.get('fila'), data.get('columna'))
        ...
        _adjuntar_ubicacion_almacen(producto)
        if almacen_error:
            producto['advertencia_almacen'] = f"No se pudo guardar la ubicación en almacén: {almacen_error}"
...
def obtener_producto(producto_id: str):
    ...
    Respuesta:
    {
        ...
        "data": {
            ...
            "codigo": "P001",
            "nombre": "Vidrio Temple 10mm",
            ...campos completos (incluyendo fila/columna si tiene almacen_id)
        }
    }
    """
    try:
        ...
        # Ubicación en almacén: productos.almacen_id -> almacen.fila/columna.
        # Antes esto se guardaba pero nunca se devolvía al cliente.
        _adjuntar_ubicacion_almacen(data)
...
def actualizar_producto(producto_id: str):
    ...
    try:
        ...
        almacen_error = None
        if data.get('fila') or data.get('columna'):
            nuevo_almacen_id, almacen_error = _crear_almacen(data.get('fila'), data.get('columna'))
            if nuevo_almacen_id:
                payload['almacen_id'] = nuevo_almacen_id
        ...
        # Ubicación en almacén actualizada (o la que ya tenía si no cambió).
        _adjuntar_ubicacion_almacen(producto)
        if almacen_error:
            producto['advertencia_almacen'] = f"No se pudo guardar la ubicación en almacén: {almacen_error}"
